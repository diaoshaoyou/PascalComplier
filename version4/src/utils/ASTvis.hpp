...
#include <fstream>
    ...
    class ASTvis {
    public:
        ASTvis(const std::string &output = "spc.tex") : of(output, of.trunc | of.out)
        {
            if (!of.is_open())
            {
    ...
    private:   
        ...
        int travExpr(const std::shared_ptr<IdentifierNode>& expr);
        int travExpr(const std::shared_ptr<ConstValueNode>& expr);
