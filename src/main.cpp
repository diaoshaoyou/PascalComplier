...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "-IR") == 0) kase = 1;
        else if (strcmp(argv[i], "-S") == 0) kase = 2;
        else if (strcmp(argv[i], "-O") == 0) optimize = true;
        else if (argv[i][0] == '-') { 
    ...
    try {
        ...
        cerr << "[CODEGEN ERROR] ";
        cerr << e.what() << endl;
        cerr << "Error during code generation" << endl;
